...
```{r distribuicoes, fig.cap="Distribuições", fig.height=6, fig.width=8, fig.align="center"}
library(patchwork)
...
```{r normal-sd, fig.cap="Duas distribuições com mesma média e variabilidades distintas.", fig.width=8, fig.height=3}
...
Para responder a isso, imaginemos o seguinte conjunto de dados, cuja média é 5. 
...
Uma pergunta que ainda não fizemos sobre o conjunto e que faz sentido colocar neste contexto é: o quão distante da média está cada uma das observações? Então, por exemplo, que número está mais próximo da média $5$: $x_1=3$ ou $x_5=7$? A resposta é óbvia, é $x_4$. E como conseguimos responder a isso? Com uma conta simples: calculando a distância da observação com a média. No caso do $x_5=7$, a conta é $7-5 = 2$; e no caso do $x_4=6$, a conta é $6 - 5 = 1$. Como $1 < 2$, então o número $6$ está mais próximo da média do que o número $7$.

Se calcularmos esse valor para todas as observações, então teremos uma informação a mais a respeito desse conjunto: teremos a “diferença” de cada observação em relação à média. Com isso, conseguimos completar o conjunto acima com a seguinte informação
...
Uma vez que sabemos como cada observação se desvia da média, falta sabermos uma última informação relevante. Lembrando que estamos falando de medidas sobre conjuntos de *informações*, é possível perceber que a informação da diferença é uma informação de *cada observação* e não do conjunto como um todo. Então o que podemos nos perguntar é: como eu trato das diferenças do *conjunto* e não de cada observação? Uma alternativa seria fazer a média das diferenças.

No entanto, a média das diferenças é sempre zero! Isso acontece porque estamos retirando a média em cada observação, e tirando a média do resultado. Isso é equivalente a retirar a média da média:

$$\frac{1}{n}\sum_{i=1}^n(x-\bar x)=\frac{1}{n}\sum_{i=1}^n x - \frac{1}{n}\sum_{i=1}^n \bar x = \bar x - \frac{n}{n}\bar x = \bar x - \bar x = 0.$$

Se ao invés de considerar as diferenças como positivas ou negativas, considerarmos apenas os valores *absolutos* das diferenças (ou seja, ignorar se o sinal é positivo ou negativo), obtemos os *desvios*. Nesse caso, a conta ficaria: $(2+1+0+1+2)/5 = 1,2$. Dessa forma podemos dizer que o nosso conjunto tem média 5 e que ele tem um *desvio médio* de $1,2$. 

Apenas uma recordação: a notação do módulo de $x$ é dada por:

$$
|x| = \left\{\begin{align}&x, \text{ se }x\geq 0,\\ -&x, \text{ se x < 0}\end{align}\right.
$$

Então, o jeito de calcular o desvio médio é calcular a média dos valores absolutos das diferenças, que são os desvios. Em notação matemática, a conta fica assim:

$$
dm(x) = \frac{1}{n}\sum_{i=1}^n\left|x-\bar x\right|
$$


Como dissemos acima, o desvio médio e o desvio padrão são medidas com a mesma lógica por detrás, mas eles se diferenciam pela forma como eles lidam com um problema matemático: os valores negativos que podem surgir da operação $x_i-x$. Enquanto o desvio médio resolve esse problema calculando o *módulo* da diferença, o desvio padrão resolve este problema elevando o desvio ao quadrado. Não existe nenhum número real que, quando elevado ao quadrado, seja negativo, por isso essa solução também resolve o problema. Então se calculássemos a diferença ao quadrado, conseguiríamos o mesmo efeito de eliminar as diferenças negativas. 

$$\text{desvio}^2=(|x-\bar x|)^2 = (x-\bar x)^2$$

Agora, por mais que os valores das observações estejam um pouco distorcidas (pois elas estão elevadas quadrado), conseguimos pelo menos preservar a ordenação. Por exemplo, voltando àquele conjunto fictício de dados de média 5, se olharmos para $x_1=3$, $x_4=6$ e $x_5=7$, veremos que as diferenças são, respectivamente $-2$, $1$ e $2$. Se olharmos para o módulo (o que seria um passo para calcular o desvio médio), veremos que o desvio de $3$ e de $7$ para a média $5$ são a mesma. Agora, se olharmos para o quadrado das diferenças, veremos que essas mesmas relações se preservam, pois $(-2)^2=4$ e $1^2=1$ e $2^2=4$. Ou seja, se o que buscávamos era eliminar os negativos de forma que conseguíssemos ainda comparar os desvios, elevar ao quadrado os desvios cumpriu a sua função perfeitamente. 

Assim, se repetirmos a fórmula do desvio médio, mas trocando o módulo para o desvio ao quadrado, teremos a seguinte fórmula:

$$
var(\boldsymbol x) = \frac{1}{n}\sum_{i=1}^n\left(x-\bar x\right)^2
$$
A fórmula acima é chamada de *variância*. A variância indica, igual ao desvio médio, o quanto as observações se distanciam da média. Entretanto – e nisso essas duas medidas se diferem entre si –, a variância indica a média dos desvios ao quadrado, e não dos desvios. 

À uma primeira vista, parece ser mais preciso utilizar o desvio médio, no lugar da variância, pois o desvio médio não distorce os números ao elevá-los ao quadrado. Entretanto, o problema causado pelo desvio médio ao calcular o módulo dos valores é muito maior. Por essa razão, a variância é preferível ao desvio médio. 

Como vimos, a variância retorna os resultados ao quadrado. Isso pode causar problemas de interpretação. Por essa razão, criamos uma medida de dispersão em torno da média que é simplesmente a raiz quadrada da variância. Essa medida é justamente o __desvio padrão__ (e cuidado para não confundir desvio padrão com desvio médio).
...
Pela fórmula do desvio padrão percebemos claramente como o desvio padrão nada mais é do que a raiz quadrada da variância. 

Com isso, conseguimos compreender o significado e as fórmulas matemáticas dessas duas medidas: o desvio médio e o desvio padrão, definindo, no meio do caminho, a variância. Os mais utilizados são a variância e o desvio padrão. O desvio padrão é mais popular do que o médio por ser mais sensível a valores atípicos (o que geralmente é desejável aqui) e por ter algumas propriedades matemáticas de interesse na modelagem estatística. Essas propriedades serão omitidas por enquanto.

Vamos antes de finalizar essa explicação das três medidas que indicam a dispersão dos dados em relação à média, dar um exemplo para dar concretude dessas medidas. O exemplo é o mesmo que vem acompanhando a gente nesta seção: o valor de avaliação dos bens alienados em processos de falência. A base usada é a base `leiloes`. Dessa vez, não vamos olhar apenas para 10 observações, mas para 483 observações da base (de um total de 1000 observações). Realizamos uma filtragem dessa base com fins didáticos apenas, para ficar mais fácil de calcular.

```{r leiloes-var}
nums <- c(126, 286, 420, 432, 750, 1660, 1900, 1949, 2500, 3700)
dados <- tibble::tibble(descricao = paste("bem", 1:10), valores = nums)
knitr::kable(
  dados, 
  caption = "Dados da base Leiloes para calcular medidas de dispersão",
  col.names = c("Descrição", "Valores")
)
```

Recomendamos fortemente que você tente calcular as duas medidas de dispersão desse conjunto. A média do valor de avaliação é R\$ 1.372,30. Tente calcular os desvios de cada um desses bens em relação à média. O resultado deve ser:

```{r leiloes-var-dif}
dados$dif <- nums - mean(nums)
knitr::kable(
  dados, 
  caption = "Dados da base Leiloes para calcular medidas de dispersão",
  col.names = c("Descrição", "Valores", "Diferença")
)
```
...
dados |> 
  dplyr::summarise(
    stat = c("desvio médio", "variância", "desvio padrão"),
    res = c(
      mean(abs(nums-mean(nums))),
      mean((nums-mean(nums))^2),
...
Vale notar, na interpretação desses resultados, que essas medidas sempre indicam dispersão ao redor da média.
...
```{r grafico-variabilidade, fig.cap="Dados de valores de avaliação com alta variabilidade."}
