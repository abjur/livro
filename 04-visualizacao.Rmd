...
[^clevelant]: Cleveland, 1985, pp. 9-10, tradução livre, grifos no original. 
...
A segunda nova comparação possível é aquela em que fixamos a variável explicativa, ou seja, a variável de quebra, e a analisamos através de todas as categorias de interesse. Vemos um exemplo disto na Figura \@ref(fig:highlight-bivar), em que olhamos para o tipo de litígio “PF-nPF” para todos os três tipos de desfechos possíveis das decisões de segunda instância. 

```{r highlight-bivar, fig.cap="Contagens de reforma/não reforma e configuração das partes."}
consumo2 <- consumo |>
  dplyr::count(dec_val, tipo_litigio) |>
  dplyr::transmute(
    dec_val = stringr::str_replace_all(dec_val, "/", "/ \n"),
...
```{r histograma-valor-2, fig.lab='Histograma de valores'}
options(scipen = 999)
consumo |>
  ggplot2::ggplot() +
  ggplot2::aes(x = valor) +
